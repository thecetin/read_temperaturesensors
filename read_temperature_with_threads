...
# Define a function that reads temperature values from multiple devices
def read_temps():
...
# Create a thread for the read_temps() function
...
while True:
  # Wait for the thread to finish and get the temperature values
    temps = temp_thread.join()
...
#Reading temperatures from sensors
    # Do something with the temperature values
    print("Temperature values:", temps)
    
    tmp1 = temps[0]#str(Read_temp(device_file1))
    
    tmp2 = temps[1]#str(Read_temp(device_file2))
    
    tmp3 = temps[2]#str(Read_temp(device_file3))
    
    # Restart the thread for the next iteration
    temp_thread = threading.Thread(target=read_temps)
    temp_thread.start()
